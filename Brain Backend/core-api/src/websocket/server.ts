...
export interface WsRoom { room: string; event: string; data: Record<string, unknown> }
...
export function createWebSocketServer(httpServer: HttpServer): SocketIOServer {
  ...
  io.use(async (socket, next) => {
    try {
      ...
      const payload = jwt.default.verify(token, config.JWT_SECRET) as { userId: string; role: string; isSeller: boolean };
  ...
  io.on('connection', (socket) => {
    ...
    socket.on('room:join', ({ room_id }: { room_id: string }) => {
    ...
    socket.on('typing:start', ({ thread_id }: { thread_id: string }) => {
      socket.to(`order:${thread_id}`).emit('typing:start', { user_id: user.userId });
    });

    socket.on('typing:stop', ({ thread_id }: { thread_id: string }) => {
      socket.to(`order:${thread_id}`).emit('typing:stop', { user_id: user.userId });
    });

    socket.on('notification:read', ({ notification_id }: { notification_id: string }) => {
  ...
  subscriber.on('message', (channel, message) => {
    if (channel !== PubSubChannels.WS_EVENTS) return;
    try {
      const { room, event, data } = JSON.parse(message) as WsRoom;
      if (io && room && event) {
