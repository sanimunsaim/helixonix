...
async function processSearchIndexing(job: Job<SearchIndexJobData>): Promise<void> {
  ...
  if (entityType === 'asset') {
    ...
    if (!asset || asset.status !== 'approved') return;
  ...
  if (entityType === 'gig') {
    ...
    if (!gig || gig.status !== 'active') return;
  ...
  if (entityType === 'seller') {
    ...
    await upsertDocument(COLLECTIONS.SELLERS, {
      ...
      bio: seller.bio ?? '',
      skills: (seller.skills as string[]) ?? [],
...
export function startSearchIndexingWorker() {
  return new Worker<SearchIndexJobData>('search-indexing', processSearchIndexing, {
