...
export const DEFAULT_MODEL_CONFIG: Record<ToolType, ModelConfig> = {
  text_to_image: {
...
export async function runModel(
  ...
  return replicate.run(modelVersion as `${string}/${string}`, { input });
}
...
export async function createPrediction(
  ...
  const prediction = await replicate.predictions.create({
    ...
    input,
    ...(webhookUrl ? { webhook: webhookUrl, webhook_events_filter: ['completed', 'failed'] as any } : {}),
  });
  return prediction as any;
}
...
): Promise<{ status: 'succeeded' | 'failed'; output: unknown; error?: string }> {
