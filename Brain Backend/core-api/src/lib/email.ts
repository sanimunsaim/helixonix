...
async function send(to: string | string[], subject: string, html: string): Promise<EmailResult> {
...
<div style="background:#13131a;border:1px solid #1e1e2e;border-radius:12px;padding:32px">
  ...
  <div style="margin-top:32px;padding-top:16px;border-top:1px solid #1e1e2e;font-size:12px;color:#64748b">
    © 2025 HelixOnix. <a href="mailto:support@helixonix.com" style="color:#6366f1">support@helixonix.com</a>
  </div>
</div></body></html>`;
}
...
export async function sendOrderConfirmationEmail(to: string, params: {
  buyerName: string; orderId: string; gigTitle: string; amount: string; deliveryDays: number;
}): Promise<EmailResult> {
...
export async function sendNewOrderEmail(to: string, params: {
  sellerName: string; orderId: string; gigTitle: string; buyerName: string; earnings: string;
}): Promise<EmailResult> {
...
export async function sendDeliveryEmail(to: string, params: {
  buyerName: string; orderId: string; gigTitle: string;
}): Promise<EmailResult> {
...
export async function sendPayoutEmail(to: string, params: {
  sellerName: string; amount: string; reference: string;
}): Promise<EmailResult> {
...
export async function sendDisputeEmail(to: string, params: {
  name: string; orderId: string; role: 'buyer' | 'seller';
}): Promise<EmailResult> {
...
export async function sendAssetApprovedEmail(to: string, params: {
  sellerName: string; assetTitle: string;
}): Promise<EmailResult> {
