...
export async function gigRoutes(app: FastifyInstance) {
  ...
  app.get('/', async (request, reply) => {
    const { category = '', page = '1' } = request.query as { category?: string; page?: string };
  ...
  app.get('/:id', async (request, reply) => {
    ...
    const gig = await db.query.gigs.findFirst({
      where: eq(gigs.id, id),
      with: { seller: { columns: { id: true, username: true, displayName: true, avatarUrl: true, sellerLevel: true, avgRating: true, completedOrders: true } } } as any,
    } as any);
    if (!gig) return reply.status(404).send({ success: false, error: { code: 'NOT_FOUND', message: 'Gig not found' } });
