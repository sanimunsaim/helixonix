...
import { redis, RedisKeys, deductCredits, addCredits, getCredits, subscriber } from '../lib/redis.js';
import { publishEvent, publishAiJobProgress, notifyUser } from '../lib/events.js';
import { AppError, ErrorCodes } from '../types/index.js';
import { DEFAULT_MODEL_CONFIG, type ToolType } from '../lib/replicate.js';
...
export async function streamJobProgress(params: {
  ...
  if (job.userId !== params.userId) throw new AppError(ErrorCodes.FORBIDDEN, 'Access denied', 403);
...
export async function refundCredits(params: { userId: string; jobId: string; creditCost: number }): Promise<void> {
...
export async function getUserGenerations(userId: string, page = 1, perPage = 20) {
