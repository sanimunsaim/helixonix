...
export async function deliverOrder(params: {
  ...
  if (order.sellerId !== params.sellerId) throw new AppError(ErrorCodes.FORBIDDEN, 'Not your order', 403);
...
export async function requestRevision(params: {
  ...
  const order = await db.query.orders.findFirst({ where: eq(orders.id, params.orderId) });
  if (!order) throw new AppError(ErrorCodes.NOT_FOUND, 'Order not found', 404);
  if (order.buyerId !== params.buyerId) throw new AppError(ErrorCodes.FORBIDDEN, 'Not your order', 403);
  if (order.status !== 'delivered') throw new AppError(ErrorCodes.ORDER_STATUS_INVALID, 'Order not in delivered status', 400);
...
export async function completeOrder(orderId: string, buyerId?: string) {
  const order = await db.query.orders.findFirst({ where: eq(orders.id, orderId) });
  if (!order) throw new AppError(ErrorCodes.NOT_FOUND, 'Order not found', 404);
  if (buyerId && order.buyerId !== buyerId) throw new AppError(ErrorCodes.FORBIDDEN, 'Not your order', 403);
  if (order.status !== 'delivered') throw new AppError(ErrorCodes.ORDER_STATUS_INVALID, 'Order not delivered yet', 400);
...
export async function submitReview(params: {
  ...
  if (!order) throw new AppError(ErrorCodes.NOT_FOUND, 'Order not found', 404);
  if (order.buyerId !== params.buyerId) throw new AppError(ErrorCodes.FORBIDDEN, 'Not your order', 403);
  if (order.status !== 'completed') throw new AppError(ErrorCodes.ORDER_STATUS_INVALID, 'Order not completed', 400);
