...
import {
  stripe, createAssetPaymentIntent, createGigPaymentIntent,
  createSubscription, transferToSeller, createRefund,
  getOrCreateStripeCustomer, constructStripeEvent
} from '../lib/stripe.js';
...
export async function checkoutAsset(params: {
  ...
  if (!asset || asset.status !== 'approved') throw new AppError(ErrorCodes.NOT_FOUND, 'Asset not found', 404);
  if (asset.isFree) throw new AppError(ErrorCodes.VALIDATION_ERROR, 'This asset is free — no purchase required', 400);
  ...
  if (existing) throw new AppError(ErrorCodes.ALREADY_EXISTS, 'You already own this asset', 409);

  const buyer = await db.query.users.findFirst({ where: eq(users.id, params.buyerId) });
  if (!buyer) throw new AppError(ErrorCodes.NOT_FOUND, 'User not found', 404);
...
export async function checkoutGig(params: {
  ...
  if (!gig || gig.status !== 'active') throw new AppError(ErrorCodes.NOT_FOUND, 'Gig not found', 404);

  const pkg = await db.query.gigPackages.findFirst({
    where: and(eq(gigPackages.gigId, params.gigId), eq(gigPackages.packageType, params.packageType as any)),
  });
  if (!pkg) throw new AppError(ErrorCodes.NOT_FOUND, 'Package not found', 404);
  ...
  if (order_type === 'asset' && asset_id && buyer_id) {
    ...
    await db.transaction(async (tx) => {
      ...
      await tx.update(users)
        .set({ availableBalance: db.execute as any }) // simplified — update via raw SQL
...
async function handleSubscriptionDeleted(sub: Stripe.Subscription) {
  ...
  const user = await db.query.users.findFirst({ where: eq(users.stripeCustomerId, sub.customer as string) });
  if (user) {
...
async function handleTransferPaid(transfer: Stripe.Transfer) {
  ...
  if (payoutId) {
    ...
    if (payout) await notifyUser(payout.sellerId, 'payout:processed', { amount: transfer.amount, reference: transfer.id });
  }
}
