...
export async function adminRoutes(app) {
    ...
    app.patch('/disputes/:disputeId/resolve', { preHandler: auth }, async (request, reply) => {
        ...
        if (!dispute)
            return reply.status(404).send({ success: false, error: { code: 'NOT_FOUND', message: 'Dispute not found' } });
    ...
    app.get('/analytics/overview', { preHandler: auth }, async (request, reply) => {
        ...
        const [orderCount] = await db.select({ count: sql `count(*)::int` }).from(orders);
        const [assetCount] = await db.select({ count: sql `count(*)::int` }).from(assets).where(eq(assets.status, 'approved'));
