...
import { stripe, createSubscription, getOrCreateStripeCustomer } from '../lib/stripe.js';
...
export async function paymentRoutes(app) {
    ...
    app.post('/subscription/create', { preHandler: [verifyJwt] }, async (request, reply) => {
        ...
        if (!priceId)
            return reply.status(400).send({ success: false, error: { code: 'VALIDATION_ERROR', message: `No price configured for plan: ${plan}` } });
    ...
    app.post('/credits/purchase', { preHandler: [verifyJwt] }, async (request, reply) => {
        ...
        if (!pack)
            return reply.status(400).send({ success: false, error: { code: 'VALIDATION_ERROR', message: 'Invalid pack' } });
        const user = await db.query.users.findFirst({ where: eq(users.id, request.user.userId) });
        if (!user)
            return reply.status(404).send({ success: false, error: { code: 'NOT_FOUND', message: 'User not found' } });
        const customerId = await getOrCreateStripeCustomer(user.id, user.email, user.displayName, user.stripeCustomerId ?? undefined);
    ...
    app.post('/webhooks/stripe', {
        ...
        if (!sig)
            return reply.status(400).send({ success: false, error: { code: 'INVALID_WEBHOOK', message: 'Missing signature' } });
        await handleStripeWebhook(request.rawBody, sig);
