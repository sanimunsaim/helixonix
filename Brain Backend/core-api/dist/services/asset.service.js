...
import { assets, assetPurchases } from '../schemas/db/index.js';
import { getSignedUploadUrl, getSignedDownloadUrl, R2Keys, ALLOWED_ASSET_TYPES } from '../lib/r2.js';
import { redis, incrementDownloadCount } from '../lib/redis.js';
...
export async function getDownloadUrl(params) {
    ...
    if (!fileKey)
        throw new AppError(ErrorCodes.NOT_FOUND, 'Asset file not available', 404);
...
export async function approveAsset(assetId, adminId) {
    ...
    await Promise.all([
        ...
        vectorEmbedQueue.add('embed-asset', { assetId, textContent: `${asset.title} ${asset.description ?? ''} ${asset.tags.join(' ')}` }),
...
export async function rejectAsset(assetId, adminId, reason) {
    const [asset] = await db.update(assets)
